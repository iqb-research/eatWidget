...
export default function ErrorChart({
  ...
  const seScale = d3
    .scaleLinear()
    .domain([0, max])
    .range([0, totalWidth / 2 - 50]);

  const est = estScale(estimate);
  const se = seScale(standardError);

  const globalEst =
    globalEstimate != undefined ? estScale(globalEstimate) : null;
  const globalSe =
    globalStandardError != undefined ? seScale(globalStandardError) : null;
  ...
  return (
    ...
    >
      ...
      {globalEst != undefined && globalSe != undefined
        ? renderErrorLine(globalEstimate, globalEst, globalSe, 5, globalColor)
        : null}
      {renderErrorLine(estimate, est, se, 3, color)}
